...
import { X, Gift, User, Phone, Mail, FileText, CheckCircle2, AlertCircle, Loader2, Sparkles, Send } from 'lucide-react';
...
export const AdminIssueComplimentaryModal: React.FC<AdminIssueComplimentaryModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!nome.trim()) {
      setErrorMsg('Informe o nome do convidado.');
      return;
    }
    ...
    try {
      const result = await createComplimentaryOrder({
        ...
        client_document: cpf.replace(/\D/g, '') || undefined,
        notes: notePayload,
          ...
          <div className="space-y-3 pt-2 border-t border-gray-100">
            <div>
              ...
              <input
                ...
                className="w-full px-3.5 py-2 text-xs font-medium rounded-xl border border-gray-300 bg-white focus:ring-2 focus:ring-emerald-500 shadow-xs"
                disabled={loading}
                required
              />
            </div>
